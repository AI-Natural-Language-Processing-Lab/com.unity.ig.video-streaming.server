...
public class RenderTextureToRTSP : MonoBehaviour
{
    public int           m_Port = 8554;
    public string        m_Username = "user";      // or use NUL if there is no username
    public string        m_Password = "password";  // or use NUL if there is no password
    public RenderTexture m_Texture;
    public int           m_FrameRate = 30;

    private RtspServer    m_Server;
    private H264Encoder   m_Encoder;
    private const float   m_StartTime = 3.0F;
    private int           m_LastFrameIndex = -1;
    private bool          m_RtspServerActive = false;
    private RenderTexture m_NV12Texture;
    private Material      m_RGBToNV12Material;

    private struct H264EncoderForStartup : IH264Encoder
    {
        public byte[] spsNalu;
        public byte[] ppsNalu;
    ...
    void Start()
    {
        ...
        try
        {
            ...
            var startJob = m_Encoder.Start((uint)m_Texture.width, (uint)m_Texture.height,
            ...
            m_NV12Texture = new RenderTexture(m_Texture.width, m_Texture.height * 3 / 2, 0, RenderTextureFormat.R8, RenderTextureReadWrite.Linear);
    ...
    {
        CompleteJobs();
        if (m_FrameRate == 0 || m_Texture == null)
            return;
        ...
        Graphics.Blit(m_Texture, m_NV12Texture, m_RGBToNV12Material);
